...
function cariPeserta(keyword) {
    ...
    return daftarPeserta.find((peserta) => peserta.nama === keyword);
}
// Output
...
function hitungRataRata(...nilai) {
    const total = nilai.reduce((hasil, n) => hasil + n, 0);
...
function buatFilterNilai(minimal) {
    return function (peserta) {
        return peserta.nilai >= minimal;
    };
}
// Menggunakan array.filter()
const pesertaLulus = daftarPeserta.filter(buatFilterNilai(85));
// Output
console.log("\n=== FILTER NILAI ===");
